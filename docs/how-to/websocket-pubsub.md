...
Pydantic V2 data payload definitions that enforce validation constraints and map database outputs. All schemas should inherit from `Zchema` and declare a `__db_name__` for domain-bound security.
...
### 🏛️ 7. Repository Layer (`repository`)
...
A generic router class that automatically generates and secures standard REST endpoints.
...
### 🛡️ 14. ASGI Middlewares (`middleware`)
...
ZCore provides Page-Number and Keyset Cursor strategies to handle dataset chunk loading.
...
### ✂️ 23. Output Projection & Security (`Zchema & response`)
...
!!! tip "💡 Fast Scaffold Lookup"
    Use the `zc startapp <app_name> -t` command when generating a new domain module to start with a pre-configured template structure, saving manual setup time.